...
ERROR_GUILD_ID = 749863506228150383
ERROR_CHANNEL_ID = 803195882224484383
...
class Giveaways(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_global(codes=[])
        self.config.init_custom(GIVEAWAY_KEY, 2)
        self.giveaways = {}
        self.codes_lock = asyncio.Lock()
    ...
    def _split_codes(self, codes: str):
        return [code.strip() for code in codes.split(",") if code.strip()]

    async def _get_codes(self):
        async with self.codes_lock:
            return await self.config.codes()

    async def _set_codes(self, codes):
        async with self.codes_lock:
            await self.config.codes.set(codes)

    async def _pop_codes(self, amount: int):
        async with self.codes_lock:
    ...
    async def _send_code_error(self, winner: discord.Member, code: str):
        ...
        if error_guild is None:
            log.error(
                "Failed to report giveaway code DM error for %s (%s): error guild not found.",
                winner.name,
                winner.id,
            )
            return
        ...
        if error_channel is None:
            log.error(
                "Failed to report giveaway code DM error for %s (%s): error channel not found.",
                winner.name,
                winner.id,
            )
            return

        message = f"Error: failed to send giveaway code. Username: {winner.name} | Discord ID: {winner.id} | Code: {code}"
        with contextlib.suppress(discord.HTTPException, discord.Forbidden):
    ...
    async def show(self, ctx: commands.Context):
        ...
        if not codes:
            return await ctx.send("No giveaway codes available.")
        message = "# Current giveaway codes:\n" + "\n".join(codes)
    ...
    @commands.guild_only()
    @commands.has_permissions(manage_guild=True)
    async def add(self, ctx: commands.Context, *, codes: str):
        """Add giveaway codes to the queue."""
        codes_to_add = self._split_codes(codes)
        if not codes_to_add:
            return await ctx.send("No codes were provided.")
        async with self.codes_lock:
            current_codes = await self.config.codes()
            await self.config.codes.set(current_codes + codes_to_add)
        await ctx.send(f"Added {len(codes_to_add)} code{'s' if len(codes_to_add) != 1 else ''}.")

    @codes.command()
    @commands.guild_only()
    @commands.has_permissions(manage_guild=True)
    async def remove(self, ctx: commands.Context, *, codes: str):
        """Remove codes from the queue."""
        codes_to_remove = self._split_codes(codes)
        ...
        async with self.codes_lock:
            ...
            for code in codes_to_remove:
                remaining_codes = [current for current in remaining_codes if current != code]
            removed_count = len(current_codes) - len(remaining_codes)
            await self.config.codes.set(remaining_codes)
        await ctx.send(f"Removed {removed_count} code{'s' if removed_count != 1 else ''}.")
    ...
    async def start(
        ...
        *,
        ...
        giveaway_obj = Giveaway(
            ...
            "🎉",
            False,
            **{"congratulate": True, "notify": True, "code_amount": 1},
        )
        if ctx.interaction:
    ...
    async def info(self, ctx: commands.Context, msgid: int):
        ...
        for kwarg in giveaway.kwargs:
            if giveaway.kwargs[kwarg]:
                display_name = kwarg.replace("_", " ").replace("-", " ").title()
                msg += f"**{display_name}:** {giveaway.kwargs[kwarg]}\n"
        embed = discord.Embed(
            title=f"{f'{winners}x ' if winners > 1 else ''}{giveaway.prize}",
            color=await ctx.embed_color(),
            description=msg,
    ...
    async def explain(self, ctx: commands.Context):
        ...
        `--code-amount`: Number of codes each winner receives in DMs. Defaults to 1.
    ...
    def generate_settings_text(self, ctx: commands.Context, args):
        ...
        if args.get("code_amount"):
            msg += f"**Code Amount:** {args['code_amount']}\n"
